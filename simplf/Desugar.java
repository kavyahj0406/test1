...
public Stmt visitForStmt(Stmt.For stmt) {
    ...
    if (stmt.init != null) {
        desugared.add(new Stmt.Expression(stmt.init));
    }

    // Create while loop with condition (default to 'true' if omitted)
    Expr condition = (stmt.cond != null) ? stmt.cond : new Expr.Literal(true);
    Stmt body = stmt.body;

    // Add increment to body if present
    if (stmt.incr != null) {
    ...
    desugared.add(new Stmt.While(condition, body));
    return new Stmt.Block(desugared);
}
